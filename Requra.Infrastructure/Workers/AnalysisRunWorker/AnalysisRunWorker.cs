...
{
    public class AnalysisRunWorker(IServiceScopeFactory _scopeFactory, IJobPollingService _jobPollingService) : IAnalysisRunWorker
    {
        public async Task ProcessRun(List<FileUploadDto> files, Guid runId, Guid projectId)
        {
            ...
            try
            {
                ...
                var AIStartJobId = await _aiClient.SubmitAsync(files,projectId.ToString(),runId.ToString());
                ...
                await _jobPollingService.PollUntilFinishedAsync(runId, AIStartJobId.JobId);
            }
            catch (Exception ex)
            {
                run.UpdateAnalysis(
        ...
        private AnalysisRunStatus MapStatus(string status)
        {
            return status switch
            {
                ...
                "REJECTED"=>AnalysisRunStatus.REJECTED,
